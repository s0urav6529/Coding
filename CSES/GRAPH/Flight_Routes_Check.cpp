...
vector<int>outtime , condense_group(limit) , scc;
    ...
    if(c > 1){
    	...
    	for(int node = 1 ; node <= n; node++){
    		...
    		for(int child: adj[node]){
    			...
    			if(condense_group[child] != condense_group[node]){
    	...
    	cout << vertex[0] <<" "<<vertex[1] <<endl;  /// if connceted component > 1
