...
void topsort(int n)
{
    ...
    while(!q.empty())
    {
        ...
        {
            ///Reduce the in degree of this current node's
            ///child because we acutually remove the current
            /// node from our graph so degree of its adjecnt node should be reduce
            degree[child]--;
            if(degree[child]==0)
            {
                q.push(child);
